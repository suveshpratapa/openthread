...
**Baseline commit:** `fb274efe6` (from upstream)

**Spec reference:** Thread 2.0 spec section 16 — Thread Direct
...
This document describes the implementation plan for the Thread Direct (TD) feature, covering the OpenThread stack, platform abstraction layer (PAL), and Spinel commands for co-processor (RCP/NCP) architectures.

### 1.1  Existing Components and Configuration

The upstream codebase at `fb274efe6` provides the scaffolding below. Several config names pre-date Chapter 16 Thread Direct terminology; this design retains them with their current names for ABI stability, adding new names only for novel Thread Direct semantics (see Decision A in §1.2).

| Component | Reuse Status | Thread Direct / Chapter 16 Mapping |
|---|---|---|
| [`OPENTHREAD_CONFIG_WAKEUP_COORDINATOR_ENABLE`](https://github.com/openthread/openthread/blob/silabs/src/core/config/wakeup.h) | Retain | WI (Wake Initiator) |
| [`OPENTHREAD_CONFIG_WAKEUP_END_DEVICE_ENABLE`](https://github.com/openthread/openthread/blob/silabs/src/core/config/wakeup.h) | Retain | WL (Wake Listener) |
| [`OPENTHREAD_CONFIG_WED_LISTEN_INTERVAL`](https://github.com/openthread/openthread/blob/silabs/src/core/config/wakeup.h) | Retain | `LISTEN_INTERVAL` (spec default: 1000 ms) |
| [`OPENTHREAD_CONFIG_WED_LISTEN_DURATION`](https://github.com/openthread/openthread/blob/silabs/src/core/config/wakeup.h) | Retain | `LISTEN_DURATION` (spec default: 8 ms) |
| [`OPENTHREAD_CONFIG_WAKEUP_COORDINATOR_CONNECTION_RETRY_INTERVAL`](https://github.com/openthread/openthread/blob/silabs/src/core/config/wakeup.h) | Retain | Retry Interval field (retry window count) |
| [`OPENTHREAD_CONFIG_WAKEUP_COORDINATOR_CONNECTION_RETRY_COUNT`](https://github.com/openthread/openthread/blob/silabs/src/core/config/wakeup.h) | Retain | Retry Count field |
| [`OPENTHREAD_CONFIG_P2P_ENABLE`](https://github.com/openthread/openthread/blob/silabs/src/core/config/p2p.h) | Retain | Thread Direct feature enable |
| [`OPENTHREAD_CONFIG_P2P_MAX_PEERS`](https://github.com/openthread/openthread/blob/silabs/src/core/config/p2p.h) | Retain | TDP peer table size |
| [`WakeupTxScheduler`](https://github.com/openthread/openthread/blob/silabs/src/core/mac/wakeup_tx_scheduler.hpp) | Reuse and extend | — |
| [`WakeupRequest`](https://github.com/openthread/openthread/blob/silabs/include/openthread/provisional/link.h) type | Reuse | — |
| [`WakeupId`](https://github.com/openthread/openthread/blob/silabs/include/openthread/provisional/link.h) (`uint64_t`) | Reuse | — |
| [`ConnectionIe` + `RendezvousTimeIe`](https://github.com/openthread/openthread/blob/silabs/src/core/mac/mac_header_ie.hpp) | Reuse wire format, can rename and extend | — |
| [`sub_mac_wed.cpp`](https://github.com/openthread/openthread/blob/silabs/src/core/mac/sub_mac_wed.cpp) — dual-path WED listen | Largely reuse | — |
| [`mle_p2p.cpp`](https://github.com/openthread/openthread/blob/silabs/src/core/thread/mle_p2p.cpp) — P2P MLE state machine | No changes required (see §3.6) | — |
| [`PeerTable` + `Peer : CslNeighbor`](https://github.com/openthread/openthread/blob/silabs/src/core/thread/peer_table.hpp) | Reuse and extend | — |
| [`HmacSha256`](https://github.com/openthread/openthread/blob/silabs/src/core/crypto/hmac_sha256.hpp) | Reuse | — |
| [`otP2pWakeupAndLink` / `otP2pUnlink` / `otP2pSetEventCallback`](https://github.com/openthread/openthread/blob/silabs/include/openthread/provisional/p2p.h) | Reuse API | — |
| [`otPlatRadioReceiveAt`](https://github.com/openthread/openthread/blob/silabs/include/openthread/platform/radio.h) | Reuse API | — |
| [`otLinkSetWakeUpListenEnabled`](https://github.com/openthread/openthread/blob/silabs/include/openthread/link.h) + params | Reuse API | — |
| [`otLinkGetWakeupChannel` / `otLinkSetWakeupChannel`](https://github.com/openthread/openthread/blob/silabs/include/openthread/link.h) | Reuse for Thread Direct | — |

New configuration flags for Thread Direct-specific behavior:

`config/wakeup.h`:

```c
// Default Wake Channel for Thread Direct per spec section 16.4.1
// Channel 20 is specified in the Thread Direct spec as the Thread Wake Channel,
// chosen to minimize interference with BLE Advertisement and Wi-Fi channels.
#ifndef OPENTHREAD_CONFIG_TD_DEFAULT_WAKE_CHANNEL
#define OPENTHREAD_CONFIG_TD_DEFAULT_WAKE_CHANNEL 20
#endif

// SLW_MIN_DURATION: minimum TD Link SLW duration (spec §16.8.3), in units of 160µs slots
#ifndef OPENTHREAD_CONFIG_TD_SLW_MIN_DURATION_SLOTS
#define OPENTHREAD_CONFIG_TD_SLW_MIN_DURATION_SLOTS 8  // 8 × 160µs ≈ 1.25ms
#endif
```
...
The existing flags `OPENTHREAD_CONFIG_WAKEUP_COORDINATOR_ENABLE` and `OPENTHREAD_CONFIG_WAKEUP_END_DEVICE_ENABLE` in `config/wakeup.h` were introduced in the initial p2p commit (4cb0de423, September 2025) before the Thread Direct spec reached its final form. They have been present in upstream for ~6 months and upstream reviewers are already familiar with them. Two options:

1. **Reuse as-is** (this design's choice): The existing flags guard the Thread Direct code paths. No config renaming is needed because the names (`WAKEUP_COORDINATOR` / `WAKEUP_END_DEVICE`) align reasonably with the spec roles (WI / WL). This avoids a downstream flag migration.
   
   or
2. **Introduce new Thread Direct flags** (`OPENTHREAD_CONFIG_TD_WAKE_INITIATOR_ENABLE`, `OPENTHREAD_CONFIG_TD_WAKE_LISTENER_ENABLE`) and deprecate the old names: gives a clean naming boundary but requires all existing users to update their build configuration.
...
1. **Compile guard** (this design's default): (`OPENTHREAD_CONFIG_TD_USE_LEGACY_WAKEUP_FRAME`): Retains the Multipurpose path for interoperability with pre-spec devices. A runtime boolean flag can then choose which wakeup format to use at runtime, supporting legacy multipurpose frames.
...
The 3-way handshake requires the WI to include the WL's Challenge IE bytes verbatim in its Enh-ACK — the WI *echoes* the received challenge (spec §16.7.3). This creates a hard real-time constraint: the IEEE 802.15.4 Enh-ACK must be generated within the hardware turnaround window after the TD Link Command arrives. SiLabs EFR32 platforms use ~192 µs in practice for Enh-ACK turnaround. The Challenge IE bytes to echo in Enh-ACK are not known until the triggering TD Link Command arrives.
We need to:
- Complete HMAC-SHA256 verification before or concurrently with Enh-ACK assembly
- In an RCP architecture, also account for round trip to the host

The spec explicitly permits sending the Enh-ACK before verification completes: *"In implementations where the challenge verification cannot be completed within the platform's ACK turnaround time, the Enh-ACK MAY be generated prior to completion of the Challenge check. Upon receiving the Enh-ACK, the WL SHALL verify the MIC, Frame Counter, and Challenge Value."* (§16.7.1).

I'm looking for advice on the implementation options, especially for RCP.

**Decision E — RCP/host support deferral** *(open question)*:

The first implementation round targets single-chip (SoC) deployments only — no host/RCP-based end device examples are currently planned. Deferring RCP host-stack support would simplify the initial scoping significantly (the Spinel `SPINEL_PROP_RCP_TD_ENH_ACK_IE` property described in §6.2 is the only RCP-specific addition). Is this acceptable to the maintainers, given that no RCP-based end device reference exists yet?

### 1.3  Open Questions for OpenThread Maintainers

The following questions are earmarked for discussion with upstream maintainers (meeting 2025-05-14):

**Q1 — Config variable naming (`OPENTHREAD_CONFIG_WAKEUP_*` vs. `OPENTHREAD_CONFIG_TD_*`)**

`config/wakeup.h` contains `OPENTHREAD_CONFIG_WAKEUP_COORDINATOR_ENABLE`, `OPENTHREAD_CONFIG_WAKEUP_END_DEVICE_ENABLE`, `OPENTHREAD_CONFIG_WED_LISTEN_INTERVAL`, etc. These were introduced in the initial [p2p] commit (4cb0de423, Sep 2025) and are the primary compile guards for all Thread Direct code. The Thread Direct spec now uses the terms Wake Initiator (WI) and Wake Listener (WL).

- Should these flags be renamed to `OPENTHREAD_CONFIG_TD_*` to align with the spec's final terminology, or should they remain as-is to avoid a migration burden on existing users?
- If renamed, what is the preferred migration path (ifdef aliasing, a deprecation release, hard cut)?
- Separately: should `OPENTHREAD_CONFIG_P2P_ENABLE` and `OPENTHREAD_CONFIG_P2P_MAX_PEERS` (`config/p2p.h`) be renamed to `OPENTHREAD_CONFIG_TD_ENABLE` / `OPENTHREAD_CONFIG_TD_MAX_PEERS`? The P2P names pre-date the "Thread Direct" branding.

**Q2 — Provisional API lifecycle (`include/openthread/provisional/`)**

The `provisional/` directory was established in the initial [p2p] commit (4cb0de423). Two headers exist: `provisional/p2p.h` (high-level P2P API: `otP2pWakeupAndLink`, `otP2pUnlink`, `otP2pSetEventCallback`) and `provisional/link.h` (`otWakeupId`, `otWakeupType`).

- What is the intended graduation path for these provisional APIs — is there a defined process or timeline for promoting them to stable (i.e., moving to `include/openthread/`)? 
- Has any previous provisional API been removed from upstream (there is no commit history of provisional header deletions)? What signals a provisional API's retirement?
- This design proposes extending `provisional/p2p.h` with `otP2pSetSlwSchedule()` and `otP2pGetPeerScaState()` (§4.2). Is adding to an existing provisional header the right approach, or should new Thread Direct APIs go to a separate `provisional/thread_direct.h`?

**Q3 — Legacy Multipurpose wake frame guard (`OPENTHREAD_CONFIG_TD_USE_LEGACY_WAKEUP_FRAME`)**

This design introduces `OPENTHREAD_CONFIG_TD_USE_LEGACY_WAKEUP_FRAME` as a compile guard that retains the legacy Multipurpose frame path alongside the new MAC Command 0x54 path (Decision C). Is this guard acceptable to maintainers, or should the old multipurpose path be removed outright at the time of the Thread Direct PR?
...
All Thread Direct frames share a single IEEE 802.15.4 MAC Command ID (`0x54`). The first payload byte after `0x54` is the **Thread MAC Command ID** (§16.5.7.4), which determines the frame's purpose and the layout of the bytes that follow:
...
- **`0x01` — Wake Command** (§16.5.7.5) — *WI → WL*
  ...
  |---|---|
  | `0x00` | TD Link establishment wake |
...
- **`0x02` — TD Link Command** (§16.9.1) — *WL → WI*
  ...
  | `0x03` | Thread Challenge IE — HMAC-SHA256 (§16.7.2) | Mandatory in TD Link Command; echoed verbatim in Enh-ACK |

A "TD Link wake" is therefore a frame with Thread MAC Command ID = `0x01` **and** Wake Frame Type = `0x00` — these are two distinct byte positions in the MAC payload. Full struct definitions and serialization detail for the Thread Header IE LTVs are in §2.2.
...
[ MAC Command Payload:
    0x54                     — IEEE 802.15.4 Command ID
    0x02                     — Thread MAC Command ID = TD Link Command
    [Link Parameter Mask]    — bitmap indicating which optional fields are present
    [Supervision Interval]   — optional; absent if not in Link Parameter Mask
    [Services bitmap]        — optional; absent if not in Link Parameter Mask
    [Short Address]          — optional (2 bytes); absent if not in Link Parameter Mask
    [Clock Accuracy/Uncertainty] — optional; absent if not in Link Parameter Mask (TBD: not yet in mask per spec)
    [Min Listen Duration]    — optional; absent if not in Link Parameter Mask (TBD: not yet in mask per spec)
]
[ MFR: MIC32 | FCS ]
```
...
2. **`TxFrame::GenerateWakeupFrame()`**:  
   ...
   The existing Multipurpose frame path **is retained** under the compile guard `OPENTHREAD_CONFIG_TD_USE_LEGACY_WAKEUP_FRAME`. When that guard is enabled, a runtime boolean (e.g., `mUseTdFrameFormat` in `WakeupTxScheduler`) selects which format to use. Both `IsWakeupFrame()` and `GenerateWakeupFrame()` contain guarded branches for each format.

3. **New `TxFrame::GenerateTdLinkCommand()`**:  
   Generates the WL → WI TD Link Command (Thread MAC Command ID = `0x02`). Encodes the mandatory short address, optional supervision interval, and services bitmap in the MAC Command payload; SCA IE and Challenge IE go in the Thread Header IE 0x2d.
...
The spec defines a new standard Header IE with element ID `0x2d` that carries LTV-encoded Thread Direct-specific elements. (Not to be confused with the already existing `ThreadIe`, which is a Vendor IE with Thread Company OUI).
...
The guest access use case requires that the Network Key (and its derived link-layer keys) is **not** shared with the TD Device acting as guest. A Wake Key is therefore independently generated and distributed only to TDP peers, keeping Thread network credentials isolated from the Wake signaling channel.
...
| Group Wake Keys | KeyIdMode2 | Wake frame addressed to a group of TD Devices |
...
The key used in the Wake Frame determines the security context for the subsequent TD Link Command and Enh-ACK:
- **Wake Key was used** → all subsequent TD Link messages use the Wake Key.
- **MAC Key was used** → all subsequent TD Link messages use the Current MAC Key.

`TdHandler` records which key type secured the incoming Wake Frame in `WakeupInfo`, and passes that context to `GenerateTdLinkCommand()` and `ComputeChallenge()`. `Mac::ProcessReceiveSecurity()` already performs key lookup by Key ID and Key ID Mode; the Wake Key at Key Index 129 should be found by the existing lookup automatically.
...
- Calls `TxFrame::GenerateWakeupFrame()` (updated per §2.1): builds a MAC Command 0x54 frame when Thread Direct format is selected, or a legacy Multipurpose frame when `OPENTHREAD_CONFIG_TD_USE_LEGACY_WAKEUP_FRAME` is active and the runtime flag chooses legacy
- Directly writes Wake Command bytes into the MAC payload: Thread MAC Command ID `0x01`, `WakeFrameType=0x00` (TD Link wake), `RendezvousTime`, then `(RI << 4) | RC` packed into **one byte** (spec §16.5.7.5: RI is bits [7:4], RC is bits [3:0] of a single octet) — follows spec §16.5.7.5 MAC payload byte layout (Thread Direct path)
- Sets security header: `KeyIdMode=1`, `KeyIndex=129` (default Wake Key), `FrameCounter` from MAC frame counter (Thread Direct path)
...
- **WakeupId wake (unicast-by-ID or group):** `DstAddr` = broadcast (0xFFFF). One or more Target ID LTVs (`kTypeTargetId = 0x01`, 1–8 bytes each) are appended inside the Thread Header IE 0x2d. Only WLs pre-configured with a matching WakeupId respond.
  ...
  - *Group wake* (requires `OPENTHREAD_CONFIG_P2P_GROUP_WAKEUP_ENABLE`): a shared WakeupId wakes multiple WLs simultaneously; uses KeyIdMode 2 (no Key Index field). All group members share the same Wake Key. `TdHandler` must handle potentially concurrent TD Link Commands arriving from multiple WLs.
...
1. **`ShouldHandleWakeupFrame()`** is already upstream (with WakeupId table filtering). The WL checks:
   - Thread Direct path: frame type == `kTypeMacCmd` AND IEEE 802.15.4 command ID == `0x54` AND Thread MAC Command ID (byte 1 of payload) == `0x01` (Wake Command). Optionally further filters on `WakeFrameType` == `0x00` (TD Link wake) by reading byte 2 of the MAC Command payload directly (no IE LTV lookup needed).
   - Legacy path (when `OPENTHREAD_CONFIG_TD_USE_LEGACY_WAKEUP_FRAME` is active): frame type == `kTypeMultipurpose` (existing behavior)
   - If `WakeupId` is present (Thread Header IE 0x2d `TargetId` LTV for Thread Direct; `ConnectionIe` for legacy), the WakeupId lookup runs against the pre-configured table
...
### 3.3  TD Link Command and `TdHandler`

The final spec uses a **MAC-layer-only TD Link Command** (MAC Command 0x54, Command=2) followed by an **Enh-ACK** from the WI. The current P2P implementation, `mle_p2p.cpp` uses **MLE Link Request / Link Accept And Request** over UDP.

The mapping is:

| Spec step | Current upstream (MLE) | Thread Direct chapter 16 spec (MAC-layer) |
|---|---|---|
...
| WL data params | MLE TLVs | TD Link Command fields: supervision, services bitmap, short addr |

**Decision B applied here (see §1.2)**: `TdHandler` is the spec-compliant path; the legacy MLE path remains available behind `OPENTHREAD_CONFIG_TD_USE_LEGACY_P2P` for PoC deployments that depend on it.

The new handshake can be implemented in:

```
src/core/mac/td_handler.hpp   // TdHandler class
src/core/mac/td_handler.cpp   // 3-way handshake state machine
```

`TdHandler` is an `InstanceLocator` and `InstanceLocator::Locator<TdHandler>` inside `Instance` (guarded by `OPENTHREAD_CONFIG_P2P_ENABLE`), following the same pattern as `WakeupTxScheduler`.

#### 3.3.1  `TdHandler` - Wake Initiator

```
States: kIdle → kWakingUp → kWaitingTdLinkCmd → kIdle
```

- `kWakingUp`: `WakeupTxScheduler` is running; when it ends, timer fires to close the connection window
- `kWaitingTdLinkCmd`: WI radio is in receive; incoming MAC Command 0x54 frames are routed via `Mac::HandleMacCommand()` → new `case Frame::kMacCmdTdLinkCommand:` → `TdHandler::HandleTdLinkCommand()`
- On TD Link Command reception:
  ...
  3. **Immediately** push the **received** Challenge IE bytes to the PAL via `otPlatRadioConfigureTdEnhAckIe()`, so the platform can insert them verbatim in the Enh-ACK (spec §16.7.3: the WI *echoes* the challenge bytes, it does not recompute them). The Enh-ACK must go out within the ~192 µs hardware turnaround window on SiLabs EFR32 — before the stack can complete HMAC verification. This timing constraint is the subject of Decision D (§1.2).
  4. **Verify ex-post:** compute the expected HMAC-SHA256 (`ComputeChallenge` in §3.3.3) and compare to the received Challenge IE bytes. The Enh-ACK has already been sent at this point.
     - If mismatch → send TD teardown frame (per spec §16.7.1; Enh-ACK suppression is not possible after the fact)
     - If match → proceed
...
- `kWaitingEnhAck`: TD Link Command has been transmitted; expecting Enh-ACK with Challenge IE
- On TD Link Command TX completion:
...
`TdHandler::GenerateTdLinkCommand()` builds the frame:

```cpp
Error TdHandler::GenerateTdLinkCommand(TxFrame &aFrame, const Peer &aPeer)
{
...
Per spec §16.7.3:
- **WL sends (TD Link Command):** `Challenge = HMAC-SHA256(Key, LinkFC ‖ WakeFC ‖ WakeID ‖ LinkSeq)`
...
`ComputeChallenge()` is therefore called on both sides for **generation/verification**, but on the WI side the output is used for comparison only; the Enh-ACK IE bytes come from the received TD Link Command.
...
```cpp
void TdHandler::ComputeChallenge(ChallengeIe          &aOut,
                                  uint32_t              aLinkFc,
                                  uint32_t              aWakeFc,
...
{
    ...
    // SCA IE fields (stored after learning from TD Link Command or Enh-ACK)
    ...
    // TD short address
    uint16_t mTdShortAddress;  ///< Assigned TD short address [0xfd00, 0xfdff], or 0xffff if unset
    ...
};
```
...
The WL includes its self-assigned TD short address in the TD Link Command. The address is selected from `[0xfd00, 0xfdff]`:

```cpp
// In TdHandler — mNextTdShortAddr is a uint16_t member of TdHandler,
// initialized to Mac::kTdShortAddrBase in the constructor.
...
uint16_t TdHandler::AllocateTdShortAddress(void)
{
    uint16_t addr = mNextTdShortAddr;
    mNextTdShortAddr = (mNextTdShortAddr < Mac::kTdShortAddrMax)
                           ? (mNextTdShortAddr + 1)
                           : Mac::kTdShortAddrBase;
    return addr;
}
```
...
```cpp
static constexpr uint16_t kTdShortAddrBase = 0xfd00;
static constexpr uint16_t kTdShortAddrMax  = 0xfdff;
```

### 3.6  `TdHandler` state machine calls

The Thread Direct 3-way handshake (Wake Frame → TD Link Command → Enh-ACK) is entirely a **MAC-layer operation**. It is different from `mle_p2p.cpp`, which is **not touched** by this feature.

`TdHandler::OnLinkEstablished()` calls `Get<PeerTable>().Add(peer)` directly. This follows the same cross-layer pattern already used in the `Mac` subsystem (e.g., `Mac` calls `Get<MeshForwarder>()` for TX scheduling). There is no MLE involvement in the Thread Direct handshake path.

MAC command dispatch in `Mac::HandleMacCommand()` gains a new case:

```cpp
case Frame::kMacCmdTdLinkCommand:  // Thread MAC Command ID = 0x02, inside a 0x54 MAC Command frame
#if OPENTHREAD_CONFIG_P2P_ENABLE
    Get<TdHandler>().HandleTdLinkCommand(aFrame);
    didHandle = true;
#endif
    break;
```

Teardown: **SCA IE with empty payload** (Length=0 in the Thread Header IE 0x2d) signals teardown per spec §16.10. `TdHandler::SendTeardown()` constructs and transmits this as a MAC Command 0x54 frame with an empty SCA LTV. The existing `otP2pUnlink()` API triggers this path.

### 3.7  Post-Link Data Transfer and Supervision

Thread Direct is a **symmetric peer-to-peer link between two TD Devices** (spec §16.1, §16.3 Design Goal "Symmetric"). Both WI and WL may be sleepy and/or CoEx-constrained; neither has its radio always on. After `OT_P2P_EVENT_LINKED` fires, each side knows the peer's SLW schedule and can schedule timed transmissions into the peer's receive windows accordingly.

**SCA IE exchange during handshake**

- **WL → WI:** The WL's SCA IE (SLW period + phase + optional RAM) is carried in the TD Link Command (§16.8). The WI records this in `Peer.mSlwPeriodSlots` / `Peer.mSlwPhaseSlots`.
- **WI → WL:** Per spec §16.8, the WI *may* include its own SCA IE in any frame — including the Enh-ACK and subsequent data frames. After receiving the TD Link Command, `TdHandler` on the WI side includes its own SCA IE in the Enh-ACK payload (if `OPENTHREAD_CONFIG_WAKEUP_COORDINATOR_ENABLE` is set and the WI itself has a configured SLW schedule). The WL records the WI's SLW schedule from the Enh-ACK or the first data frame.
...
1. `MeshForwarder` has a pending frame addressed to the WL's TD short address.
2. A new `TdTxScheduler` computes the next SLW window start time from `Peer.mSlwPhaseSlots` and `Peer.mSlwPeriodSlots`, using the same epoch-relative phase tracking that `CslTxScheduler::GetNextTxDelay()` uses for CSL.
...
- **WI side:** `TdHandler` starts a `mSupervisionTxTimer` (fires at `supervision_interval / 2` after each delivery). If no data is pending when the timer fires, `TdHandler::SendSupervisionFrame()` sends a zero-payload MAC frame to the WL's SLW window. This is directly analogous to `ChildSupervisor` (`child_supervisor.cpp`). *Note: the spec requires one supervision frame if no TX/RX occurred within the full Supervision Interval; firing at half the interval is an intentional conservative choice that sends extra keepalives at roughly 2× the required rate, trading a small overhead for earlier link-loss detection on the WL's RX timer.*
- **WL side:** `TdHandler` starts a `mSupervisionRxTimer`, reset on each received frame from the WI. On expiry, link loss detection triggers (§16.10.6): the WL drops the connection and returns to wake listen mode (see Teardown below).
...
In all cases the `Peer` entry is removed from `PeerTable`, freeing the TD short address slot for reuse.
...
Some functions in `include/openthread/provisional/p2p.h` **already mirror the proposed Thread Direct API**:
...
/**
 * Represents the SCA (Scheduled Channel Access) schedule state of a TDP peer.
 */
...
/**
 * Gets the SCA schedule of a TDP peer.
 *
 * @param[in]  aInstance     The OpenThread instance.
 ...
 *       Spinel property (SPINEL_PROP_RCP_TD_ENH_ACK_IE) to push the configuration over
 *       the transport.
 *
 * @param[in] aInstance        OpenThread instance.
 * @param[in] aSrcExtAddress   Extended address of the TD Link Command sender (peer WL).
 ...
 */
otError otPlatRadioConfigureTdEnhAckIe(otInstance         *aInstance,
...
```
SPINEL_PROP_THREAD_TD__BEGIN = SPINEL_PROP_THREAD__BEGIN + 0x60  (provisional range)
```

| Property | ID | Format | Description |
|---|---|---|---|
| `SPINEL_PROP_THREAD_TD_ENABLE` | +0x00 | `b` | Enable/disable Thread Direct (mirrors `OPENTHREAD_CONFIG_P2P_ENABLE`) |
| `SPINEL_PROP_THREAD_TD_MAX_PEERS` | +0x01 | `S` | Max peers (read-only) |
| `SPINEL_PROP_THREAD_TD_WAKEUP_CHANNEL` | +0x02 | `C` | Wake channel (maps to `otLinkSetWakeupChannel`) |
| `SPINEL_PROP_THREAD_TD_WL_LISTEN_ENABLE` | +0x03 | `b` | Enable WL listening (`otLinkSetWakeUpListenEnabled`) |
| `SPINEL_PROP_THREAD_TD_WL_LISTEN_PARAMS` | +0x04 | `LL` | interval_us, duration_us (`otLinkSetWakeupListenParameters`) |
| `SPINEL_PROP_THREAD_TD_WI_WAKEUP` | +0x05 | `t(...)` | Trigger WI wake+link, returns done asynchronously |
| `SPINEL_PROP_THREAD_TD_UNLINK` | +0x06 | `E` | Trigger WL unlink by ExtAddress |
| `SPINEL_PROP_THREAD_TD_PEER_TABLE` | +0x07 | `A(t(...))` | Read peer table (array of peer info) |
| `SPINEL_PROP_THREAD_TD_EVENT` | +0x08 | `CE` | Async notify: event code + peer ExtAddress |
| `SPINEL_PROP_THREAD_TD_SLW_SCHEDULE` | +0x09 | `SS` | period_slots, phase_slots (`otP2pSetSlwSchedule`) |

**`SPINEL_PROP_THREAD_TD_WI_WAKEUP` format** (TLV encoded in `t()`):
...
Response: `SPINEL_CMD_PROP_VALUE_IS` on `SPINEL_PROP_THREAD_TD_EVENT` with link-done event.

### 6.2  RCP Architecture (host runs OT stack, RCP is the radio)

In the RCP architecture, the host stack handles all Thread Direct logic.  The RCP only needs standard radio operations.  The **sole new Spinel property required for the RCP** is the Enh-ACK IE injection, defined as a direct parallel to the existing `SPINEL_PROP_RCP_ENH_ACK_PROBING` (at `SPINEL_PROP_RCP_EXT__BEGIN + 3`):

```c
// spinel.h — in the SPINEL_PROP_RCP_EXT range
SPINEL_PROP_RCP_TD_ENH_ACK_IE = SPINEL_PROP_RCP_EXT__BEGIN + 6,
```

| Property | Format | Direction | Description |
|---|---|---|---|
| `SPINEL_PROP_RCP_TD_ENH_ACK_IE` | `Ed` | host → RCP | Configure Enh-ACK IE: ExtAddress (8B) + raw Thread Header IE payload bytes. (Spinel type codes: `E` = EUI-64 8-byte address, `d` = length-prefixed raw data.) |

**`radio_spinel.cpp` binding** — new method maps to `otPlatRadioConfigureTdEnhAckIe()`:

```cpp
otError RadioSpinel::ConfigureTdEnhAckIe(const otExtAddress &aSrcExtAddress,
                                          const uint8_t      *aIeData,
                                          uint16_t            aIeDataLength)
{
    return Set(SPINEL_PROP_RCP_TD_ENH_ACK_IE,
...
No other new Spinel properties are needed for the RCP case — all other Thread Direct operations (wake frame TX, WED ReceiveAt, standard data TX to peer) use existing Spinel mechanisms.
...
# Peer table
p2p peers                # List all valid TDP peers
```
...
| Thread MAC Command sub-namespace: Wake Frame Type 0x02 (Connectionless) vs. TD Link Command 0x02 | §16.5.7.4 gap | Gap acknowledged in spec; using `0x01`=Wake Command and `0x02`=TD Link Command in the Thread MAC Command ID field; within Wake Command payload, Wake Frame Type `0x02` (connectionless) does not collide since it is at a different byte position. Connectionless Wake is still a well-defined type per §16.5.2. |
| Group wake TX/RX (`OPENTHREAD_CONFIG_P2P_GROUP_WAKEUP_ENABLE`) | §16.5 | Is group wake spec final? |
| eCSL slot contention between TD Link SLW windows and regular CSL windows | §16.9 | Slot algorithm needs to be finalized in spec. Can follow `GetNeededShift()` pattern |
